...
int main()
{
    ...
    if (serial_port < 0)
    {
        printf("Don't read or write file");
    }
    ...
    while (1)
    {
        printf("Type quit to exit and type any to continious\n");
        printf("Please insert input: ");
        memset(&msg, '\0', sizeof(msg));
        memset(&read_buf, '\0', sizeof(read_buf));
        fgets(msg, sizeof(msg), stdin); // input from keyboard
        printf("String that inputed: %s\n",msg);
        ret = write(serial_port, msg, sizeof(msg));
        if (ret != sizeof(msg))
        {
            printf("Fail writing and Value return: %d\n", ret);
        }
        else if (strcmp((char *)msg, "quit\n") == 0)
        {
            break;
        }
        else
