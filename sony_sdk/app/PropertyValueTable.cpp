...
namespace cli
{
    ...
    std::vector<std::int16_t> parse_exposure_compensation(unsigned char const *buf, std::uint32_t nval)
    {
        using TargetType = std::int16_t;
        constexpr std::size_t const type_size = sizeof(TargetType);
        TargetType const *source = reinterpret_cast<TargetType const *>(buf);
    ...
    text format_exposure_compensation(std::int16_t exposure_compensation)
    {
        text_stringstream ts;
        float value = (float)exposure_compensation / 1000.0f;
        ts << value << "Ev";
        return ts.str();
    }
