...
void CameraDevice::get_exposure_compensation()
{
    load_properties();
    tout << "Exposure Compensation: " << format_exposure_compensation(m_prop.exposure_compensation.current) << '\n';
}
...
void CameraDevice::set_exposure_compensation()
{
    
    if (1 != m_prop.exposure_compensation.writable) {
        // Not a settable property
        tout << "Exposure Compensation is not writable\n";
        return;
    }

    text input;
    tout << "Would you like to set a new Exposure Compensation value? (y/n): ";
    std::getline(tin, input);
    if (input != TEXT("y")) {
        tout << "Skip setting a new value.\n";
        return;
    ...
    tout << "Choose a number set a new Exposure Compensation value:\n";
    tout << "[-1] Cancel input\n";

    auto& values = m_prop.exposure_compensation.possible;
    for (std::size_t i = 0; i < values.size(); ++i) {
        tout << '[' << i << "] " << format_exposure_compensation(values[i]) << '\n';
    }
    ...
    if (prop_list && nprop > 0) {
        ...
        for (std::int32_t i = 0; i < nprop; ++i) {
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_ExposureBiasCompensation:
                nval = prop.GetValueSize() / sizeof(std::int16_t);
                m_prop.exposure_compensation.writable = prop.IsSetEnableCurrentValue();
                m_prop.exposure_compensation.current = static_cast<std::int16_t>(prop.GetCurrentValue());
                if (0 < nval) {
                    auto parsed_values = parse_exposure_compensation(prop.GetValues(), nval);
                    m_prop.exposure_compensation.possible.swap(parsed_values);
                }
                break;
            default:
                break;
